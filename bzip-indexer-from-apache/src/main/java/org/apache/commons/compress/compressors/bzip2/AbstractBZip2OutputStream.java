...
public abstract class AbstractBZip2OutputStream extends CompressorOutputStream
    ...
    // Fist block has one more byte than expected somehow. Compensating this strange behaviour.
    protected int blockBytesWritten = -1;
    ...
    protected int bsBuff;
    protected int bsLive;
     ...
     */
    public AbstractBZip2OutputStream(final OutputStream out)
            throws IOException {
     ...
     * @see #MAX_BLOCKSIZE
     */
    public AbstractBZip2OutputStream(final OutputStream out, final int blockSize) throws IOException {
        if (blockSize < 1) {
    ...
    protected OutputStream getInnerOutputStream() {
        return out;
    }
    ...
    /**
     * Workaround to count one extra byte in last block, which is
     */
    protected boolean isBlockNotFull() {
        return last < allowableBlockSize;
    }
    ...
    protected int getRunLength() {
        return runLength;
    }

    protected abstract void onEndOfBlock() throws IOException;
